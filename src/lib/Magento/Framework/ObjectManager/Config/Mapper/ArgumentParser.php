 ...
 */
class Magento_Framework_ObjectManager_Config_Mapper_ArgumentParser
{
    /**
     * @var Magento_Framework_Config_Converter_Dom_Flat
     */
    private $converter;
    ...
    {
        if (!$this->converter) {
            $arrayNodeConfig = new Magento_Framework_Config_Dom_ArrayNodeConfig(
                new Magento_Framework_Config_Dom_NodePathMatcher(),
               array('argument(/item)+' => 'name')
            );
            $this->converter = new Magento_Framework_Config_Converter_Dom_Flat($arrayNodeConfig);
        }
